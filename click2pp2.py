...
def move_baxter(pose_x, pose_y, pose_z, mode='pick'):
	robot = baxter()
 	if mode == 'pick' : 
	    position = Point(x = 0.725 - pose_y, y= - 0.3 - pose_x + 0.0175, z= 0.49 - pose_z - 0.05)
	else :
	...
	try:
		while True:
			...
			if key & 0xFF == ord('q') or key == 27:
			    ...
			    #depth_pixel = [515 - int(start_pix_x * 380/256.), 400 - int(start_pix_y * 380/256.)]
			    ...
			    point = rs.rs2_deproject_pixel_to_point(intrin, depth_pixel, depth)
			    print(point)
			    move_baxter(point[0], point[1], point[2],'place')
			    ...
			    print(point)
			    position = Point(x = 0.700 - pose_y, y= - 0.3 - pose_x + 0.0175, z= 0.49 - pose_z - 0.025)
			    orientation = Quaternion(x=0.0,y=1.0,z=0.0,w=0.0)
			    start_pose = Pose(position=position, orientation=orientation)
			    print(start_pose)
			    pick_return = robot.pick(start_pose)
			    
			    #depth_pixel = [515 - int(goal_pix_x * 380/256.), 400 - int(goal_pix_y * 380/256.)]
